...
fn make_app() -> App {
    let config = Config {
        ...
        model_overrides: llm_manager::config::ModelConfigStore::new(vec![]),
...
fn test_parse_tensor_count_updates() {
    ...
    app.add_log("llama_model_loader: loading tensor  1 of  640, n_loaded 1", llm_manager::config::LogLevel::Info);
    assert_eq!(app.loading.load_progress.tensors_loaded, 1);
    assert_eq!(app.loading.load_progress.tensors_total, Some(640));

    app.add_log("llama_model_loader: loading tensor  320 of  640, n_loaded 320", llm_manager::config::LogLevel::Info);
...
fn test_parse_multiple_buffer_sizes() {
    ...
    app.loading.loading_phases.insert(LoadingPhase::LoadingTensors);
    app.loading.last_active_phase = Some(LoadingPhase::LoadingTensors);
    app.add_log("Vulkan0 model buffer size =  3285.54 MiB", llm_manager::config::LogLevel::Info);
    app.add_log("CPU model buffer size =  6571.09 MiB", llm_manager::config::LogLevel::Info);
    app.add_log("kv buffer size =  4194.30 MiB", llm_manager::config::LogLevel::Info);
...
fn test_full_loading_sequence_phases() {
    ...
    app.add_log("http server listening on port 49222", llm_manager::config::LogLevel::Info);
    assert!(app.loading.loading_phases.contains(&LoadingPhase::ServerListening));
}
...
fn test_full_loading_sequence_details() {
    ...
    app.add_log("offloading 32 repeating layers to GPU", llm_manager::config::LogLevel::Info);
    assert_eq!(app.loading.load_progress.layers_total, Some(32));
    ...
    app.add_log("offloaded 16/32 layers to GPU (3285.54 MiB)", llm_manager::config::LogLevel::Info);
    assert_eq!(app.loading.load_progress.layers_loaded, Some(16));
...
fn test_error_triggers_reset() {
    ...
    app.model_states.insert("test-model".to_string(), ModelState::Loading);

    // Send an error
    app.add_log("ERROR: out of memory allocated_id=0", llm_manager::config::LogLevel::Error);
...
fn test_error_not_loading_model_unchanged() {
    ...
    app.model_states.insert("test-model".to_string(), ModelState::Available);

    // Send an error
    app.add_log("ERROR: something went wrong", llm_manager::config::LogLevel::Error);

    // Model should still be Available (error detection only triggers during loading)
    assert!(matches!(app.model_states.get("test-model"), Some(ModelState::Available)));
}

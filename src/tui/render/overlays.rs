...
fn render_bench_tune_setup(
    ...
    let data_rows: Vec<Row> = config
        ...
        .map(|(i, p)| {
            ...
            Row::new(vec![
                Cell::from(Span::styled(marker, Style::default().fg(ACCENT))),
    ...
    let footer_lines = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled(" [Alt+M]", Style::default().fg(ACCENT)),
            Span::raw(" Mode "),
            Span::styled(" [Alt+N]", Style::default().fg(ACCENT)),
            Span::raw(" Tokens "),
            Span::styled(" [Alt+I]", Style::default().fg(ACCENT)),
            Span::raw(" Iters "),
...
fn render_max_concurrent_picker(f: &mut Frame, area: Rect, app: &App, value: &str) {
    ...
    picker_lines.push(Line::from(vec![
        Span::styled(ctx_len.to_string(), Style::default().fg(ACCENT)),
        Span::raw(" / "),
        Span::styled(
...
fn render_dashboard_picker(
    ...
    picker_lines.push(Line::from(vec![
        Span::styled(auth_marker, Style::default().fg(ACCENT)),
        Span::styled(
...
fn render_bench_tune_output(f: &mut Frame, area: Rect, app: &App, result_idx: usize) {
    ...
    if let Some(result) = results.get(result_idx) {
        ...
        let param_rows: Vec<Row> = vec![
            ...
            Row::new(vec![
                Cell::from(Span::styled("threads", Style::default().fg(ACCENT))),
                Cell::from(Span::styled(
        ...
        let controls = Line::from(vec![
            ...
            Span::raw("  "),
            Span::styled("[j/k] scroll  ", Style::default().fg(ACCENT)),
            Span::raw("  "),

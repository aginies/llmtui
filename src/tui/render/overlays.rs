...
pub fn render_overlays(f: &mut Frame, app: &mut App) -> bool {
    if app.ui.panel_help {
        ...
        let help_area = Rect { x: (area.width - w) / 2, y: (area.height - h) / 2, width: w, height: h };
    ...
    if let GlobalMode::CmdLine { cmd_line } = &app.ui.global_mode {
        ...
        let block = Block::default().title(" CmdLine — Esc to close  e to export ").borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow));
    ...
    if let GlobalMode::PromptPicker { entries, selected, editing, edit_buffer, edit_cursor_pos, confirm_delete } = &app.ui.global_mode {
        render_prompt_picker(f, f.area(), entries, *selected, *editing, edit_buffer, *edit_cursor_pos, *confirm_delete);
        return true;
    }
...
fn render_confirmation(f: &mut Frame, area: Rect, app: &App, selected: bool, kind: ConfirmationKind) {
    let popup_area = Rect { x: area.width.saturating_sub(50) / 2, y: area.height.saturating_sub(8) / 2, width: 50, height: 8 };
    let (title, text_lines) = match kind {
        ConfirmationKind::Exit => {
            ...
            (" Exit Application? ", vec![
                Line::from(""),
                Line::from(vec![Span::raw("There are "), Span::styled(format!("{}", loaded_count), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)), Span::raw(" model(s) loaded.")]),
                Line::from("Are you sure you want to exit?"),
            ])
        }
        ConfirmationKind::Reset => (" Reset Settings? ", vec![Line::from(""), Line::from("Reset all LLM settings to defaults?")]),
        ConfirmationKind::Delete => {
            let model_name = app.selected_model().map(|m| m.name.as_str()).unwrap_or("Unknown");
            (" Delete Model? ", vec![Line::from(""), Line::from(vec![Span::raw("Are you sure you want to delete "), Span::styled(model_name, Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)), Span::raw("?")]), Line::from(""), Line::from("This action cannot be undone.")])
        }
        ConfirmationKind::Unload => {
            let model_name = match &app.pending.pending_api_unload { Some((name, _)) => name.as_str(), None => "Unknown" };
            (" Unload Model? ", vec![Line::from(""), Line::from(vec![Span::raw("Unload "), Span::styled(model_name, Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)), Span::raw("?")])])
        }
        ConfirmationKind::DeleteBackend => {
            let (backend, tag) = match &app.pending.pending_backend_deletion { Some((b, t)) => (b.to_string(), t.as_str()), None => ("Unknown".to_string(), "latest") };
            (" Delete Backend? ", vec![Line::from(""), Line::from(vec![Span::raw("Delete backend "), Span::styled(format!("{} ({})", backend, tag), Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)), Span::raw("?")]), Line::from(""), Line::from("This will remove the binary and shared libraries.")])
        }
    };
    let block = Block::default().title(title).borders(Borders::ALL).border_style(Style::default().fg(if kind == ConfirmationKind::Delete || kind == ConfirmationKind::DeleteBackend { Color::Red } else { Color::Yellow }));
    ...
    f.render_widget(Paragraph::new(lines).block(block).alignment(Alignment::Center), popup_area);
}
...
fn render_host_picker(f: &mut Frame, area: Rect, entries: &Vec<(String, String)>, selected: usize) {
    ...
    picker_lines.push(Line::from(Span::styled(" [↑] Select Host Address  [d] Refresh  [j/k] nav  [Esc] cancel ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
    ...
    for (i, (ip, iface)) in entries.iter().enumerate() {
        ...
        picker_lines.push(Line::from(vec![Span::styled(marker, Style::default().fg(Color::Yellow)), Span::styled(format!("{ip}"), style), Span::raw("  "), Span::styled(format!("({iface})"), Style::default().fg(Color::DarkGray))]));
    }
...
fn render_profile_picker(f: &mut Frame, area: Rect, entries: &Vec<(String, String)>, selected: usize, current_settings: &crate::models::ModelSettings, selected_profile: Option<&crate::config::Profile>) {
    ...
    for (i, (name, desc)) in entries.iter().enumerate() {
        ...
        let style = if i == selected { Style::default().fg(Color::Black).bg(Color::Yellow).add_modifier(Modifier::BOLD) } else { Style::default().fg(Color::White) };
        let display_name = if is_builtin { format!("{} (built-in)", name) } else { name.clone() };
        picker_lines.push(Line::from(vec![Span::styled(marker, Style::default().fg(Color::Yellow)), Span::styled(display_name, style)]));
        if !desc.is_empty() {
...
fn render_prompt_picker(f: &mut Frame, area: Rect, entries: &Vec<(String, String)>, selected: usize, editing: bool, edit_buffer: &str, _edit_cursor_pos: usize, confirm_delete: bool) {
    let w = (area.width as f64 * 0.7).clamp(60.0, 80.0) as u16;
    let h = if editing { (area.height as f64 * 0.8).clamp(25.0, 40.0) as u16 } else { (area.height as f64 * 0.7).clamp(20.0, 35.0) as u16 };
    let picker_area = Rect { x: (area.width - w) / 2, y: (area.height - h) / 2, width: w, height: h };
    let mut picker_lines: Vec<Line> = Vec::new();
    if confirm_delete && selected < entries.len() {
        ...
        let display_name = if is_builtin { format!("{} (built-in)", name) } else { name.clone() };
        picker_lines.push(Line::from(Span::styled(format!(" Delete '{}'?", display_name), Style::default().fg(Color::Red).add_modifier(Modifier::BOLD))));
        picker_lines.push(Line::from(""));
        picker_lines.push(Line::from(Span::styled(" [Y] Yes  [N] Cancel  [Esc] Cancel ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
        picker_lines.push(Line::from(""));
    } else if editing {
        picker_lines.push(Line::from(Span::styled(format!(" Editing: {}", if selected < entries.len() { &entries[selected].0 } else { "New Preset" }), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
        picker_lines.push(Line::from(""));
        ...
        for line in content_lines.iter().take(max_lines) {
            ...
            if in_range {
                ...
                picker_lines.push(Line::from(vec![Span::raw(before), Span::styled("|", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)), Span::raw(after)]));
            } else {
        ...
        picker_lines.push(Line::from(""));
        picker_lines.push(Line::from(Span::styled("[Enter] new line  [Esc] cancel  [Ctrl+S] save", Style::default().fg(Color::Cyan))));
    } else {
        picker_lines.push(Line::from(Span::styled(" [↑/↓] Select  [Enter] Confirm  [e] Edit  [n] New  [d] Delete  [Esc] Cancel ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
        picker_lines.push(Line::from(""));
    ...
    f.render_widget(Clear, picker_area);
    f.render_widget(Paragraph::new(picker_lines).wrap(Wrap { trim: true }).block(Block::default().title(" Prompt Presets ").borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow))), picker_area);
}

fn render_tags(f: &mut Frame, area: Rect, app: &App) {
    let w = (area.width as f64 * 0.5).clamp(40.0, 60.0) as u16;
    let h = (app.settings.tags.len() + 8).min(area.height as usize - 4) as u16;
    let modal_area = Rect { x: (area.width - w) / 2, y: (area.height - h) / 2, width: w, height: h };
    let mut modal_lines: Vec<Line> = Vec::new();
    modal_lines.push(Line::from(Span::styled(" Tags Editor ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
    modal_lines.push(Line::from(""));
    if app.edit.tags_insert_mode {
        modal_lines.push(Line::from(Span::styled(" [Enter] Add tag  [Esc] Cancel  [Tab] Switch to edit mode ", Style::default().fg(Color::DarkGray))));
    } else {
        modal_lines.push(Line::from(Span::styled(" [e/i] Edit  [d/Del] Delete  [a] Add  [Tab] Switch to add mode ", Style::default().fg(Color::DarkGray))));
    }
    modal_lines.push(Line::from(""));
    for (i, tag) in app.settings.tags.iter().enumerate() {
        let marker = if Some(i) == app.edit.tags_selected_idx { "> " } else { "  " };
        let style = if Some(i) == app.edit.tags_selected_idx { Style::default().fg(Color::Black).bg(Color::Yellow).add_modifier(Modifier::BOLD) } else { Style::default().fg(Color::White) };
        modal_lines.push(Line::from(vec![Span::styled(marker, Style::default().fg(Color::Yellow)), Span::styled(tag.clone(), style)]));
    }
    if app.edit.tags_insert_mode {
        modal_lines.push(Line::from(vec![Span::styled(" New: ", Style::default().fg(Color::Yellow)), Span::styled(&app.edit.tags_edit_buffer, Style::default().fg(Color::Black).bg(Color::Yellow)), Span::styled("_", Style::default().fg(Color::Black).bg(Color::Yellow))]));
    } else if app.edit.tags_selected_idx.is_some() {
        modal_lines.push(Line::from(vec![Span::styled(" Edit: ", Style::default().fg(Color::Yellow)), Span::styled(&app.edit.tags_edit_buffer, Style::default().fg(Color::Black).bg(Color::Yellow)), Span::styled("_", Style::default().fg(Color::Black).bg(Color::Yellow))]));
    }
    f.render_widget(Clear, modal_area);
    f.render_widget(Paragraph::new(modal_lines).block(Block::default().title(" Tags Editor ").borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow))), modal_area);
}
...
fn render_backend_picker(f: &mut Frame, area: Rect, entries: &Vec<(crate::models::Backend, Option<String>)>, selected: usize) {
    ...
    for (i, (backend, tag)) in entries.iter().enumerate() {
        ...
        let display_label = if let Some(t) = tag { format!("{} ({})", label, t) } else { format!("{} (latest/auto)", label) };
        let mut line_spans = vec![Span::styled(marker, Style::default().fg(Color::Yellow)), Span::styled(display_label, style)];
...
fn render_bench_tune_setup(f: &mut Frame, area: Rect, app: &App, config: &crate::models::BenchTuneConfig, selected_idx: usize, bench_mode_selection: usize, editing_prompt: bool) {
    ...
    let block = Block::default().title(Span::styled(" Benchmark Configuration ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))).borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow));
    let inner_area = block.inner(popup_area);
    f.render_widget(Clear, popup_area);
    f.render_widget(block, popup_area);
    let regions = ratatui::layout::Layout::vertical([
        ratatui::layout::Constraint::Length(1), ratatui::layout::Constraint::Length(1), ratatui::layout::Constraint::Length(1),
        ratatui::layout::Constraint::Length(5), ratatui::layout::Constraint::Length(1), ratatui::layout::Constraint::Length(1),
        ratatui::layout::Constraint::Min(0), ratatui::layout::Constraint::Length(3),
    ]).split(inner_area);
    let iters_display = if app.edit.editing_iters { format!("{}|", app.edit.iters_edit_buffer) } else { config.num_iterations.to_string() };
    let tokens_display = if app.edit.editing_n_predict { format!("{}|", app.edit.n_predict_edit_buffer) } else { config.n_predict.to_string() };
    let mode_line = Line::from(vec![
        Span::styled(" Mode: ", Style::default().fg(Color::Yellow)), Span::styled(mode_name, Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
        Span::raw(" | "), Span::styled("Iters: ", Style::default().fg(Color::Yellow)),
        ...
        Span::raw(" | "), Span::styled("Max Tokens: ", Style::default().fg(Color::Yellow)),
    ...
    let prompt_lines = if editing_prompt {
        ...
        vec![Line::from(display_text), Line::from(""), Line::from(vec![Span::styled(format!(" [{} chars] ", config.prompt.len()), Style::default().fg(Color::Cyan)), Span::styled(" [Press Esc to finish] ", Style::default().fg(Color::Yellow))])]
    } else {
...
fn render_rpc_manager(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    let rpc_area = Rect { x: (area.width.saturating_sub(w)) / 2, y: (area.height.saturating_sub(h)) / 2, width: w, height: h };
    let workers = &app.config.rpc_workers;
    let worker_lines = panel::rpc_workers::render_all(workers, app.picker.rpc_workers_selected_idx, app.picker.editing_rpc_worker.is_some(), &app.settings_state.settings_edit_buffer, app.edit.edit_cursor_pos);
    ...
    let visible_lines: Vec<Line> = worker_lines.iter().skip(start_idx).take(available_height as usize).cloned().collect();
    let block = Block::default().title(" RPC Workers Manager ").borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));
...
fn render_about_overlay(f: &mut Frame, area: Rect) {
    ...
    let h = 16;
    let about_area = Rect { x: (area.width.saturating_sub(w)) / 2, y: (area.height.saturating_sub(h)) / 2, width: w, height: h };
    let about_lines = panel::about::render_about();
    let block = Block::default().title(" About ").borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));
    f.render_widget(Clear, about_area);
    f.render_widget(Paragraph::new(about_lines).block(block).alignment(Alignment::Center), about_area);
}
...
fn render_max_concurrent_picker(f: &mut Frame, area: Rect, app: &App, value: &str) {
    ...
    let per_model = if entered > 0 && ctx_len > 0 { ctx_len / entered } else { ctx_len };
    let mut picker_lines: Vec<Line> = Vec::new();
    picker_lines.push(Line::from(Span::styled(" Max Concurrent Predictions ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
    picker_lines.push(Line::from(""));
    picker_lines.push(Line::from(vec![Span::raw("This divides the context length per loaded model: ")]));
    ...
    picker_lines.push(Line::from(""));
    picker_lines.push(Line::from(vec![Span::raw("Value: "), Span::styled(value, Style::default().fg(Color::Black).bg(Color::Yellow))]));
    picker_lines.push(Line::from(""));
    picker_lines.push(Line::from(vec![
...
fn render_dashboard_picker(f: &mut Frame, area: Rect, _app: &App, enabled: bool, port: &str, auth_key: &str, tls_enabled: bool, tls_cert: &str, tls_key: &str, selected_field: i32, editing: bool, edit_buffer: &str) {
    ...
    picker_lines.push(Line::from(vec![Span::styled(enabled_marker, Style::default().fg(Color::Yellow)), Span::styled("Enabled: ", Style::default().fg(Color::Yellow)), Span::styled(if enabled { "On" } else { "Off" }, Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))]));
    picker_lines.push(Line::from(""));
    let port_marker = if selected_field == 0i32 { "> " } else { "  " };
    let port_val = if editing && selected_field == 0i32 { format!("{}|", edit_buffer) } else { port.to_string() };
    picker_lines.push(Line::from(vec![Span::styled(port_marker, Style::default().fg(Color::Yellow)), Span::styled("Port: ", Style::default().fg(Color::Yellow)), Span::styled(port_val, Style::default().fg(Color::White))]));
    picker_lines.push(Line::from(""));
    ...
    f.render_widget(Clear, picker_area);
    f.render_widget(Paragraph::new(picker_lines).block(Block::default().title(" Dashboard ").borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow))), picker_area);
}
...
fn render_dashboard_url(f: &mut Frame, area: Rect, app: &App, host: &str, port: &str, auth_key: &str, ws_enabled: bool) {
    ...
    if !auth_key.is_empty() { url.push_str(&format!("?auth={}", auth_key)); }
    let mut picker_lines: Vec<Line> = Vec::new();
    picker_lines.push(Line::from(Span::styled(" Server Dashboard ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
    picker_lines.push(Line::from(""));
    picker_lines.push(Line::from(vec![Span::styled("Host: ", Style::default().fg(Color::Yellow)), Span::styled(host_val, Style::default().fg(Color::White))]));
    picker_lines.push(Line::from(""));
    picker_lines.push(Line::from(vec![Span::styled("Backend: ", Style::default().fg(Color::Yellow)), Span::styled(&backend_str, Style::default().fg(Color::White))]));
    picker_lines.push(Line::from(vec![Span::styled("Threads: ", Style::default().fg(Color::Yellow)), Span::styled(&threads_str, Style::default().fg(Color::White))]));
    picker_lines.push(Line::from(vec![Span::styled("Threads Batch: ", Style::default().fg(Color::Yellow)), Span::styled(&threads_batch_str, Style::default().fg(Color::White))]));
    picker_lines.push(Line::from(vec![Span::styled("Mode: ", Style::default().fg(Color::Yellow)), Span::styled(&mode_str, Style::default().fg(Color::White))]));
    picker_lines.push(Line::from(vec![Span::styled("API Endpoint: ", Style::default().fg(Color::Yellow)), Span::styled(api_str, Style::default().fg(Color::White))]));
    picker_lines.push(Line::from(vec![Span::styled("RPC Workers: ", Style::default().fg(Color::Yellow)), Span::styled(&rpc_str, Style::default().fg(Color::White))]));
    picker_lines.push(Line::from(vec![Span::styled("Dashboard: ", Style::default().fg(Color::Yellow)), Span::styled(if ws_enabled { "Enabled" } else { "Disabled" }, Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))]));
    picker_lines.push(Line::from(""));
    picker_lines.push(Line::from(Span::styled(&url, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))));
    picker_lines.push(Line::from(""));
    picker_lines.push(Line::from(vec![Span::styled("[Enter] copy URL  ", Style::default().fg(Color::Black).bg(Color::DarkGray)), Span::styled("[Esc] close", Style::default().fg(Color::Black).bg(Color::DarkGray))]));
    f.render_widget(Clear, picker_area);
    f.render_widget(Paragraph::new(picker_lines).block(Block::default().title(" Server Dashboard ").borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow))), picker_area);
}
...
fn render_bench_tune_output(f: &mut Frame, area: Rect, app: &App, result_idx: usize) {
    ...
    if let Some(result) = results.get(result_idx) {
        let modal_area = Rect { x: 0, y: 0, width: area.width, height: area.height };
        f.render_widget(Clear, modal_area);
        let p_str = if format_bench_params(&result.params, false).is_empty() { "Baseline".to_string() } else { format_bench_params(&result.params, false).join(", ") };
        let main_title = Line::from(vec![Span::styled(" BenchTune Result: ", Style::default().fg(Color::Yellow)), Span::styled(p_str, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))]);
        let settings = result.base_settings.as_ref();
        let param_rows: Vec<Row> = vec![
            Row::new(vec![Cell::from(Span::styled("temperature", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| format!("{:.2}", s.temperature)).unwrap_or_else(|| result.params.temperature.map(|v| format!("{:.2}", v)).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("top_p", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| format!("{:.2}", s.top_p)).unwrap_or_else(|| result.params.top_p.map(|v| format!("{:.2}", v)).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("top_k", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| s.top_k.to_string()).unwrap_or_else(|| result.params.top_k.map(|v| v.to_string()).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("repeat_penalty", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| format!("{:.2}", s.repeat_penalty)).unwrap_or_else(|| result.params.repeat_penalty.map(|v| format!("{:.2}", v)).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("context_length", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| s.context_length.to_string()).unwrap_or_else(|| result.params.context_length.map(|v| v.to_string()).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("batch_size", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| s.batch_size.to_string()).unwrap_or_else(|| result.params.batch_size.map(|v| v.to_string()).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("flash_attn", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| if s.flash_attn { "on".to_string() } else { "off".to_string() }).unwrap_or_else(|| result.params.flash_attn.map(|v| if v { "on".to_string() } else { "off".to_string() }).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("threads", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| s.threads.to_string()).unwrap_or_else(|| result.params.threads.map(|v| v.to_string()).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
           Row::new(vec![Cell::from(Span::styled("expert_count", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| s.expert_count.to_string()).unwrap_or_else(|| result.params.expert_count.map(|v| v.to_string()).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("spec_type", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.as_ref().map(|s| if s.spec_type.is_empty() { "-".to_string() } else { s.spec_type.clone() }).unwrap_or_else(|| result.params.spec_type.as_ref().map(|s| if s.is_empty() { "-".to_string() } else { s.clone() }).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("draft_tokens", Style::default().fg(Color::Yellow))), Cell::from(Span::styled(settings.map(|s| s.draft_tokens.to_string()).unwrap_or_else(|| result.params.draft_tokens.map(|v| v.to_string()).unwrap_or_else(|| "-".to_string())), Style::default().fg(Color::Cyan)))]),
        ];
        let params_table = Table::new(param_rows, [Constraint::Fill(1), Constraint::Fill(1)])
            .header(Row::new(vec![Cell::from(Span::styled("Parameter", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))), Cell::from(Span::styled("Value", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)))]))
            .block(Block::default().title(" Parameters ").borders(Borders::ALL).border_style(Style::default().fg(Color::Cyan)));
        ...
        let metric_rows: Vec<Row> = vec![
            Row::new(vec![Cell::from(Span::styled("prompt_tps", Style::default().fg(Color::Green))), Cell::from(Span::styled(format!("{:.2}", metrics_for_output.prompt_tps), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("gen_tps", Style::default().fg(Color::Green))), Cell::from(Span::styled(format!("{:.2}", metrics_for_output.generation_tps), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("combined_tps", Style::default().fg(Color::Green))), Cell::from(Span::styled(format!("{:.2}", metrics_for_output.combined_tps), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("latency/token", Style::default().fg(Color::Green))), Cell::from(Span::styled(format!("{:.2} ms", metrics_for_output.latency_per_token), Style::default().fg(Color::Cyan)))]),
            Row::new(vec![Cell::from(Span::styled("first_token", Style::default().fg(Color::Green))), Cell::from(Span::styled(format!("{:.2} ms", metrics_for_output.first_token_time), Style::default().fg(Color::Cyan)))]),
        ];
        let metrics_table = Table::new(metric_rows, [Constraint::Fill(1), Constraint::Fill(1)])
            .header(Row::new(vec![Cell::from(Span::styled("Metric", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))), Cell::from(Span::styled("Value", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)))]))
            .block(Block::default().title(format!(" Metrics (Iter {}/{}) ", output_idx + 1, result.outputs.len())).borders(Borders::ALL).border_style(Style::default().fg(Color::Cyan)));
        let output_lines: Vec<Line> = if !result.outputs.is_empty() { result.outputs[output_idx].lines().map(|l: &str| Line::from(l.to_string())).collect() } else { vec![Line::from("No output captured.")] };
        let title_area = Rect { x: 0, y: 0, width: area.width, height: 1 };
        f.render_widget(Paragraph::new(main_title).alignment(Alignment::Center), title_area);
        let top_height = 11;
        let left_width = area.width / 2;
        let right_width = area.width - left_width;
        let left_area = Rect { x: 0, y: 1, width: left_width, height: top_height };
        f.render_widget(params_table, left_area);
        let metrics_area = Rect { x: left_width, y: 1, width: right_width, height: top_height };
        ...
        if output_y < area.height.saturating_sub(controls_height) {
            let output_area = Rect { x: 0, y: output_y, width: area.width, height: area.height.saturating_sub(output_y + controls_height) };
            let output_block = Block::default().title(" Captured Output ").borders(Borders::ALL).border_style(Style::default().fg(Color::Cyan));
            if !output_lines.is_empty() {
                let scroll = app.bench_tune.bench_tune_output_scroll as u16;
                f.render_widget(Paragraph::new(output_lines).block(output_block).wrap(Wrap { trim: false }).scroll((scroll, 0)), output_area);
            } else {
        ...
        for (r_idx, r) in results.iter().enumerate() {
            if r_idx < result_idx { absolute_idx += r.outputs.len(); }

...
pub fn render_about() -> Vec<Line<'static>> {
    ...
    lines.push(Line::from("A high-performance TUI for managing llama.cpp servers."));
    lines.push(Line::from(""));
    lines.push(Line::from(vec![
        Span::raw("License: "),
        Span::styled("GNU GPLv3", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
    ]));
    lines.push(Line::from(""));
    lines.push(Line::from("This is free software: you are free to change and redistribute it."));
    lines.push(Line::from("There is NO WARRANTY, to the extent permitted by law."));
    lines.push(Line::from(""));
    lines.push(Line::from(vec![
    ...
    lines.push(Line::from(""));
    lines.push(Line::from(vec![
        Span::styled("[Esc/Any] Close", Style::default().fg(Color::DarkGray)),
    ]));

...
pub fn render_all(settings: &crate::models::ModelSettings, cached: &crate::models::ModelSettings, selected: usize, edit_buf: &str, editing: bool, vram_mib: u64, total_layers: u32, n_ctx_train: u32, max_threads: u32) -> (Vec<Line<'static>>, usize, usize, usize) {
    ...
    // Add max context hint below Context (if n_ctx_train is known)
    if n_ctx_train > 0 {
        lines.push(Line::from(vec![
            Span::styled(format!("  (max: {n_ctx_train} tokens)", n_ctx_train = n_ctx_train), Style::default().fg(Color::DarkGray)),
        ]));
    }
    // Add max threads hint below Threads
    lines.push(Line::from(vec![
        Span::styled(format!("  (max: {max_threads} cores)", max_threads = max_threads), Style::default().fg(Color::DarkGray)),
    ]));
...
pub fn add_setting(lines: &mut Vec<Line<'static>>, total_count: &mut usize, settings: &crate::models::ModelSettings, cached: &crate::models::ModelSettings, name: &str, val: &str, selected: usize, edit_buf: &str, editing: bool) {
    ...
    let dirty = match current_idx {
        ...
        6 => settings.flash_attn != cached.flash_attn,
        7 => settings.kv_cache_offload != cached.kv_cache_offload,
        8 => settings.cache_type_k != cached.cache_type_k,
        9 => settings.cache_type_v != cached.cache_type_v,
        10 => settings.batch_size != cached.batch_size,
        11 => settings.parallel != cached.parallel,
        12 => settings.uniform_cache != cached.uniform_cache,
        ...
        21 => (settings.presence_penalty - cached.presence_penalty).abs() > 0.001,
        22 => (settings.frequency_penalty - cached.frequency_penalty).abs() > 0.001,
        _ => false,
    };

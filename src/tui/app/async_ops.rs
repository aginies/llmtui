...
impl App {
    pub fn cancel_download(&mut self, idx: usize) {
        ...
        self.download.download_progress[idx].status = crate::models::DownloadStatus::Cancelled;
    ...
    pub async fn process_pending_download(
        ...
        if models_dir.is_absolute() {
            models_dir = tokio::fs::canonicalize(&models_dir)
                .await
        ...
        if !dest_canonical.starts_with(&models_dir) {
            self.add_log(
                format!(
                    "Download destination {} escapes models directory",
                    dest.display()
                ),
                crate::config::LogLevel::Warning,
            );
    ...
    pub async fn process_pending_deletion(&mut self, path: PathBuf) {
            ...
            && let Ok(mut entries) = tokio::fs::read_dir(parent).await
        {
            ...
            if !has_files && let Err(e) = tokio::fs::remove_dir(parent).await {
                self.add_log(
    ...
    pub fn process_pending_backend_deletion(
        &mut self,
    ...
    pub async fn tick_web_search_check(&mut self) {
            ...
            && let Some(handle) = self.pending.web_search_check_handle.take()
        {
            match handle.await {
                Ok(Ok(())) => {
                    if let crate::tui::app::GlobalMode::WebSearchPicker { check_status, .. } =
                        &mut self.ui.global_mode
                    {
                ...
                Err(e) => {
                    ...
                    {
                        *enabled = false;
                        *check_status = Some(crate::tui::app::WebSearchCheckStatus::Error(
                ...
                match &state.status {
                    crate::models::DownloadStatus::Complete => {
                        if state.model_id == "llama-server" {
                            ...
                            let search_results: &Vec<crate::models::SearchResult> =
                                match &self.models_mode {
                            ...
                            if let crate::tui::app::ModelsMode::Search { results, .. } =
                                &mut self.models_mode
                            {
                                for result in results.iter_mut() {
                                    result.downloaded = super::sync_ops::model_is_downloaded(
                                        &self.models,
                                        &result.model_id,
                                    );
                                }
    ...
    pub fn tick_server_logs(&mut self) {
        ...
        if let Some(rx) = &mut self.server.server_log_rx {
            ...
            while let Ok(line) = rx.try_recv() {
                let (metrics, is_generation) =
                    server_logs::parse_log_line(&line, prev_line.as_deref());
                ...
                if let Some(v) = metrics.prompt_tokens {
                    self.metrics.prompt_tokens = v;
                }
                if let Some(v) = metrics.prompt_progress {
                    self.metrics.prompt_progress = v;
                }
                if let Some(v) = metrics.prompt_elapsed_ms {
                    self.metrics.prompt_elapsed_ms = v;
                }
                if let Some(v) = metrics.prompt_tps_eval {
                    self.metrics.prompt_tps_eval = v;
                }
                if let Some(v) = metrics.ctx_used {
                    self.metrics.ctx_used = v;
                }
                if let Some(v) = metrics.decoded_tokens {
                    self.metrics.decoded_tokens = v;
                }
                if let Some(v) = metrics.gen_tps {
            ...
            if let Some(rx) = &mut self.loading.loading_completion_rx {
                ...
                if got_completion {
                        ...
                        && !self.config.default.web_search_engine_url.is_empty()
                    {
                        ...
                        tokio::spawn(async move {
                            if let Err(e) =
                                crate::backend::web_search::check_health(&engine_url, &api_key)
                                    .await
                            {
                                ...
                                if let Err(save_err) = config.save() {
                                    tracing::error!(
                                        "Failed to save config after web search disable: {}",
                                        save_err
                                    );
                                }
                            }
                                ...
                                while let Ok(models) = rx.try_recv() {
                                    for (id, status, _) in models {
                                        ...
                                        if status_lower == "loaded" || status_lower == "ready" {
                                            ...
                                            let name =
                                                self.model_states.iter().find_map(|(name, s)| {
                                                    if matches!(
                                                        s,
                                                        crate::models::ModelState::Loading
                                                    ) && id == *name
    ...
    pub async fn tick_spawn_result(&mut self, handle: SpawnTaskHandle) {
        match handle.await {
            Ok(Ok((server_display_name, server_handle, cmd, spawned_settings))) => {
                ...
                self.server.spawned_context_length =
                    (spawned_settings.context_length as f32 * spawned_settings.rope_scale) as u32;
    ...
    async fn metrics_polling_task(
        ...
        loop {
            ...
            let mut m = match crate::backend::server::get_metrics(
                &host,
                port,
                current_model_for_metrics.as_deref(),
                Some(pid),
            )
            .await
            {
                ...
                Ok(bench_results) => {
                        ...
                        && let Some(model) = self.selected_model()
                    {
                        ...
                        let task_handle = tokio::spawn(async move {
                            let _ = crate::backend::server::unload_model(&host, port, &model_name)
                                .await;
                        });
            ...
            && let Some(handle) = &self.server.server_handle
        {
            ...
            if server_mode == crate::models::ServerMode::Normal {
                ...
                self.background_tasks.insert(
                    ...
                    tokio::spawn(async move {
                        if let Err(e) =
                            crate::backend::server::unload_model(&host, port, &model_id).await
                        {

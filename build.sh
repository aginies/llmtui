...
examples() {
    ...
    echo "  $0 serve --model model.gguf --host 0.0.0.0  # Bind to all interfaces"
    echo "  $0 serve --model model.gguf --backend-binary /opt/rocm/bin/llama-server  # Custom binary"
    echo "  $0 serve --model model.gguf --log-file /var/log/llm-manager/model.log  # Log to file"
...
cmd_gnome_ext() {
    ...
    gnome-extensions pack "$ext_src" -f
